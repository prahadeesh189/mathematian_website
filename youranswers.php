...
<button id="askbtn" value="add" onclick="askbtn()" >Ask Question</button>
...
<?php
	...
	$q = "SELECT * FROM `question` WHERE questioned_by = $curr_uID " ;
...
function askbtn(){
	window.open("http://localhost/upld_q.php?current_qID=" + current_qID +"&current_uID="+sessionStorage.getItem('current_uID'),"_self");			
}
...
function goingtoview(click_id){
	...
	if (current_qID > 0 ) {
		window.location.href = "http://localhost/viewpage.php?current_qID=" + current_qID +"&current_uID="+sessionStorage.getItem('current_uID');
	} else 
		exit();
